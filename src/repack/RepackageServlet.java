...
public class RepackageServlet extends HttpServlet {
	...
	public static String baseUrl = "http://119.29.15.79:8080/game/";
	public static String testtomcat = "F:/tomcat7/apache-tomcat-7.0.57/webapps/game/";//
	public static String realtomcat = "D:/apache-tomcat-7.0.63_pack/webapps/game/";//"E:/apache-tomcat-7.0.61/webapps/game/";
	public static String disk = "D";
	public static String workFile = "decode";
	...
	private void handleRequest(HttpServletRequest request,
		...
		finalfilePacket = fileName.replaceAll("\\.apk", ""); // ("\\.apk",
																// "");//
																// +"_mzyw";
		packetIds = request.getParameter("packetIds");

		System.out.println("****************打包参数*******************");
		System.out.println("packetIds:" + packetIds);
		...
		if (fileName.isEmpty() || packetIds.isEmpty()) {
			...
			for (int i = 0; i < len; i++) {
				...
				finalfileNames[i] = finalfilePacket + "_" + channels[i]
						+ gameId + packetNo + ".apk";
			}
			...
			if (commandFileName != null) {
				result = runRepackTask(commandFileName, finalfilePacket,
						finalfileNames);
		...
		if (result.equals(SUCCESS)) {
			for (int j = 0; j < finalfileNames.length; j++) {
				String url = baseUrl + finalfilePacket + "/"
						+ finalfileNames[j];
	...
	private String createCommandFile(String fileName, String finalfilePacket,
			...
			throws IOException {
		String commandFileName = disk+":/"+workFile+"/exe_" + gameId + ".bat";
