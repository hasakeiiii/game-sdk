...
function qm_create(sd, v, ts, th, oc, rl, sh, fl, nf, l) {
    ...
    if (!l) {
        l = 1;
        qm_th = th;
        sd = document.getElementById("nav");
    ...
    for (var i = 0; i < sp.length; i++) {
        ...
        if (b.tagName == "DIV") {
            if (window.showHelp && !window.XMLHttpRequest) sp[i].insertAdjacentHTML("afterBegin", "<span class='nav_clear'>&nbsp;</span>");
            x2("qmparent", lsp, 1);
            lsp.cdiv = b;
            b.idiv = lsp;
...
function qm_a(a) {
    if (a[qc].indexOf("nav") + 1) return 1;
};
function qm_uo(a, go) {
