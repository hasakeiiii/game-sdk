...
function cal_WebCalendar(){ //初始化日历的设置
    ...
    this.dateElem=null;
    this._jsKind="";//自定义变量，使控件能够按月或者天,周自动修改endDate
 ...
 //   debugger;
	...
	if (arguments.length == 0){ 
        ...
        WebCalendar.objExport = eval(arguments[0]);
     	...
     	WebCalendar._jsKind=arguments[1];
    }
...
function cal_returnDate(){ //根据日期格式等返回用户选定的日期
    if(WebCalendar.objExport)
    {
        ...
        var nowDateStr=dateToStr(nowDateTemp);
        var nowDate=strToDate(nowDateStr);  //默认时间带有时分秒比较时候需要清空
        var selectDateStr=WebCalendar.objExport.value;
        var selectDate=strToDate(selectDateStr);
        //如果选择是开始日期控件
        if(WebCalendar.objExport.id=="startDate"){
          	//如果选择时间大于今天,则开始和结束日期都为当天
          	if(selectDate>nowDate){
          		cal_getObjectById("startDate").value=nowDateStr;
          		if(cal_getObjectById("endDate")!=null){
          			cal_getObjectById("endDate").value=nowDateStr;
          		}
          	}
          	else{
          	    if(cal_getObjectById("endDate")!=null){
	          	    //选择不是当前月份,之前已经判断过大于当天，所以这里肯定是往月的。
	          	    if(WebCalendar.thisYear+"-"+WebCalendar.thisMonth==(nowDateTemp.getFullYear()+"-"+(nowDateTemp.getMonth()+1))){
		          		  //根据要求修改日期结束控件时间
		          		  if(WebCalendar._jsKind=="day"){
				      	  	cal_getObjectById("endDate").value=cal_getObjectById("startDate").value;
				      	  }
				      	  else if(WebCalendar._jsKind=="week"){
				      	  	cal_getObjectById("endDate").value=nowDateStr;
				      	  }
				      	  else{
				      	  	cal_getObjectById("endDate").value=nowDateStr;
		          	...
		          	//往月数据
		          	    ...
		          	    //结束时间为当月最大一天
		          	    var outPutDay=selectDate.getFullYear()+"-"+(selectDate.getMonth()+1)+"-";
		          	    ...
		          	    if(month==1 || month==3 || month==5 || month==7 || month==8 || month==10 || month==12){
		          	    	...
		          	    	var tempDays=(0==y%4 && (y%100!=0 || y%400==0)) ? 29 : 28;
		          	    	outPutDay+=tempDays;
		          	    }
		          	    else{
		          	    	outPutDay+="30";
		          	    }
		          		cal_getObjectById("endDate").value=formatDate(outPutDay);
		            }
          	    }  
          	} 
         }
        //检查是否是日期结束控件，是的话如果时间大于当前时间换为当前时间
        else if(WebCalendar.objExport.id=="endDate"){
           //如果选择时间大于今天,则结束日期都为当天
           if(selectDate>nowDate){
          		cal_getObjectById("endDate").value=nowDateStr;
          	}
          	//判断结束时间是否小于起始时间,如果是则开始时间等于结束时间
          	if(cal_getObjectById("startDate")!=null && strToDate(cal_getObjectById("startDate").value)>selectDate){
         		cal_getObjectById("startDate").value=selectDateStr;
         	}
          }
        cal_hiddenCalendar();
    }
...
//提交按钮上直接调用_jsSearch('chargeListMonth.jsp') 提交的form必须叫mainForm
function _jsSearch(){
	var local="";
	if (arguments.length == 0){ 
	    local=location.href.lastIndexOf("?") == -1 ? location.href.substring((location.href.lastIndexOf("/")) + 1) : location.href.substring((location.href.lastIndexOf("/")) + 1, location.href.lastIndexOf("?"));
	}
    else{
        local=arguments[0];
    }
	...
	document.mainForm.action=local;
