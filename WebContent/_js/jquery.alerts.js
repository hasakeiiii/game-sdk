...
(function($) {
	...
	$.alerts = {
		...
		ok: function(message, title, callback) {
			if( title == null ) title = '系统提示';
			$.alerts._show(title, message, null, 'ok', function(result) {
				if( callback ) callback(result);
			});
		},
		...
		_show: function(title, msg, value, type, callback) {
			...
			switch( type ) {
				case 'error':
					...
					$("#popup_message").after('<div id="popup_panel"><img src=\"../_js/ico/btn_ok.gif\" border=\"0\" id="popup_ok" style=\"cursor:pointer\"/></div>');
	...
	ok = function(message,callback,title) {
		$.alerts.ok(message, title, callback);
	}
