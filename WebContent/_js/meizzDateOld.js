...
function WebCalendar()  
{
      ...
      this.dayBgColor = "#FF9E47";      //日期数字背景色
      this.todayColor = "#FF0000";      //今天在日历上的标示背景色
      this.DarkBorder = "#D4D0C8";      //日期显示的立体表达色,日期显示边框颜色 
      
      
      this.inputDateColor    = "#0066FF";      //控件的输入框日期的颜色
      this.dateElem=null;
      this.myMonthMaxDay=30;
       //自定义变量，使控件能够按月或者天自动修改endDate
      this._jsDay="";
}   
...
//调用方法。第一个参数代表调用方法的控件，第二个参数代表开始，结束时间关联性。如果参数2是day则为同日关联否则是同月关联
function calendar() //主调函数
{
     if(arguments.length==2){
     	WebCalendar._jsDay=arguments[1];
     }
      var e = window.event.srcElement;     writeIframe();
      WebCalendar.dateElem=e;
...
while (e = e.offsetParent){t += e.offsetTop; l += e.offsetLeft;}
      ...
      try{
          //原有方法trim有异常，重新替换
          var myTempDate=WebCalendar.objExport.value.replace(/(^[\s]*)|([\s]*$)/g, "");
          if (myTempDate != ""){
              WebCalendar.dateStyle = myTempDate.match(WebCalendar.dateReg);
...
function writeCalendar() //对日历显示的数据的处理程序
{
      ...
      WebCalendar.myMonthMaxDay=m==1    ? WebCalendar.daysMonth[11] : WebCalendar.daysMonth[m-1];
      ...
      for(var i=0; i<WebCalendar.dayShow; i++)      //这个循环是根据源数组写到日历里显示
      {
          ...
          {WebCalendar.dayObj[i].bgColor = WebCalendar.inputDateColor; WebCalendar.dayObj[i].style.color = WebCalendar.lightColor;}
...
function returnDate() //根据日期格式等返回用户选定的日期
{
      if(WebCalendar.objExport)
      {
          ...
          //为了使用方便选择开始日期以后另外一个结束日期控件跟着改变
          if(WebCalendar.dateElem.id=="startDate" && getObjectById("endDate")!=null){
          	//如果选择时间是本月,而且选择时间大于今天,则结束日期为当天
          	if(WebCalendar.thisYear+"-"+WebCalendar.thisMonth==(new Date().getFullYear()+"-"+(new Date().getMonth()+1))){
          		if(strToDate(getObjectById("endDate").value)>strToDate(WebCalendar.thisYear+"-"+WebCalendar.thisMonth+"-"+WebCalendar.thisDay)){
          			getObjectById("endDate").value=WebCalendar.thisYear+"-"+WebCalendar.thisMonth+"-"+new Date().getDate();
          		}
          	}
          	//选择不是当前月份
          	else{
          		var d = strToDate(getObjectById("startDate").value);
          		getObjectById("endDate").value=d.getFullYear()+"-"+(d.getMonth()+1)+"-"+new Date(d.getFullYear(), (d.getMonth()+1), 0).getDate();
          	} 
         }
         /*
      	  if(WebCalendar._jsDay=="day"){
      	  	getObjectById("endDate").value=getObjectById("startDate").value;
      	  }
      	  else if(WebCalendar._jsDay=="week"){
      	  	getObjectById("endDate").value=WebCalendar.thisYear+"-"+WebCalendar.thisMonth+"-"+WebCalendar.thisDay;
      	  }
      	  else{
      	  	getObjectById("endDate").value=WebCalendar.thisYear+"-"+WebCalendar.thisMonth+"-"+WebCalendar.thisDay;
      	  }
      	  **/
          //检查是否是日期结束控件，是的话如果时间大于当前时间换为当前时间
          if(WebCalendar.dateElem.id=="endDate"){
          	if((strToDate(getObjectById("endDate").value).getMonth()+1)==(new Date().getMonth()+1)){
          		if(strToDate(getObjectById("endDate").value)>strToDate(WebCalendar.thisYear+"-"+WebCalendar.thisMonth+"-"+WebCalendar.thisDay)){
          			getObjectById("endDate").value=WebCalendar.thisYear+"-"+WebCalendar.thisMonth+"-"+new Date().getDate();
          		}
          	}
          ...
          //最后如果日期不是标准日期就换成标准日期
          getObjectById("endDate").value=formatDate(getObjectById("endDate").value);
          hiddenCalendar();
      }
}
...
//提交按钮上直接调用_jsSearch('chargeListMonth.jsp') 如果有参数就直接跟在url上
function _jsSearch(src){
    ...
    if(!patrn.exec(startDate.value)){
		alert("起始日期格式不正确！");
        return;
    }
     //检查开始时间是否比现在大
    if(strToDate(startDate.value)>new Date()){
		alert("起始日期大于当前日期！");
        return;
    }
    if(endDate.value==""){
    ...
    if(startDate.value.substring(0,7)!=endDate.value.substring(0,7)){
        alert("不支持跨月查询，请选择同年同月日期！");
        return;
    }
	document.mainForm.action=src;
