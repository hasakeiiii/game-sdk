...
/* 	Map typeMap=new HashMap<String,String>();
	...
	if((!StringUtil.is_nullString(op))&&(op.equals("sch"))){
	    ...
	    list = dao.getAppListPayList(fromDate, toDate, BSS_NO, gameNo, channelNo, pay_id);
	...
	<form name="mainForm" method="post" style="margin:0;padding:0">
		...
		<table id="TableColor" width="100%" border="0">
			...
			for(int i=0;i<list.size();i++){
					...
					String gameName = "";
					String channelName = "";
					Pay data = (Pay)list.get(i);
					gameno = (String)data.getGameId();
				...
				%>
				<tr>
				<td><%=data.getDate()+"-"+data.getTime() %></td>
				<td><%=gameName %></td>
				<td><%=channelName %></td>
				<td><%=packetid %></td>
				<td><%=amount/100 %>元</td>
				<td><%=data.getPayType() %></td>
...
<script type="text/javascript" src="../_js/jquery.alerts.js"></script>
 ...
 <script>
	function fromyearchg(obj){
		document.mainForm.action="payListDataReq.jsp";
	...
	function goSearch(){
		document.mainForm.action="payListDataReq.jsp?op=sch";
		document.mainForm.submit();
	}
</script>
