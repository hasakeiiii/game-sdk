...
<%
DebuUtil.log("cfgGamePayDataList");
...
ArrayList<GamePayData> list = new ArrayList<GamePayData>();
int recordcount = 0;
...
if((!StringUtil.is_nullString(op)) &&(op.equals("sch")))//
{
	...
	if(channel_id != -1)
	{
		...
		list = GamePayData.get_GamedataList(null,null,null,cooperation.packet_id,cooperation.settle_type,distype,beginDate,endDate);
	...
	else
	{
		...
		list = GamePayData.get_GamedataList(game_no,business_no,channel_no,null,null,distype,beginDate,endDate);
	}
...
for(int i=0;i<list.size();i++){
	...
	long datedif = 0;
	GamePayData gamedata=(GamePayData)list.get(i);
	...
	String yeepaystr = String.format("%.2f",gamedata.yeePay/100.0);
	String alipaystr = String.format("%.2f",gamedata.aliPay/100.0);
	String allpaystr = String.format("%.2f",(gamedata.mmPay+gamedata.yeePay+gamedata.aliPay)/100.0);
%>
<tr>
<td><%=datestr%></td>
<td><%=mmpaystr%></td>
...
String yeepaycountstr = String.format("%.2f", yeepaycount/100.0);
String alipaycountstr = String.format("%.2f", alipaycount/100.0);
String allpaycountstr = String.format("%.2f", (mmpaycount+yeepaycount+alipaycount)/100.0);
...
{
	//alert(obj.value);
	document.mainForm.action="cfgGamePayDataList.jsp?op=selgame";
	document.mainForm.submit();
}
function businesschg(obj)
{
	//alert(obj.value);
	document.mainForm.action="cfgGamePayDataList.jsp?op=selbusiness";
	document.mainForm.submit();
}
function channelchg(obj)
{
	document.mainForm.action="cfgGamePayDataList.jsp?op=selchannel";
	document.mainForm.submit();
}

function packetchg(obj)
{
	document.mainForm.action="cfgGamePayDataList.jsp?op=sel";
	document.mainForm.submit();
}
...
function goSearch(){
	...
	document.mainForm.action="cfgGamePayDataList.jsp?op=sch";
	document.mainForm.submit();
}
</script>
