...
if(!keyWord.equals("")){
	 //  pageSql+=" and settle_type like '%"+keyWord+"%'";
}
...
<td><%=cooperation.getSettleType()%></td>
<td><%=cooperation.getSettleValue()%></td>
<td><%=cooperation.getSettleRatio()%></td>
<td><%=businesser_name%></td>
<% if(gameType.equals("on_line")){%>
	<td><%=cooperation.getDiscontent()%></td>
<% }else{ %>
<td></td>
<% }%>
<td><a href="#" onclick="telec('<%=cooperation.getId()%>','<%=app_name%>','<%=channel_name%>','<%=cooperation.getPacketId()%>')">电信计费点</a></td>
<td><a href="#" onclick="unic('<%=cooperation.getId()%>','<%=app_name%>','<%=channel_name%>','<%=cooperation.getPacketId()%>')">联通计费点</a></td>
<td><a href="#" onclick="mm('<%=cooperation.getId()%>','<%=app_name%>','<%=channel_name%>','<%=cooperation.getPacketId()%>')">移动计费点</a></td>
<td><a href="#" onclick="remind('<%=cooperation.getId()%>','<%=app_name%>','<%=channel_name%>','<%=cooperation.getPacketId()%>')">配置报警监控信息</a></td>
...
<script>
function telec(id,appName,channelName,packetId){
	window.location="../servlet/CooperationTelAction?cmd=remind&cooperationId=" + id +"&appName=" + appName + "&channelName=" + channelName + "&packetId=" + packetId;
}
function unic(id,appName,channelName,packetId){
	window.location="../servlet/CooperationUnAction?cmd=remind&cooperationId=" + id +"&appName=" + appName + "&channelName=" + channelName + "&packetId=" + packetId;
}
function mm(id,appName,channelName,packetId){
	window.location="../servlet/CooperationMmAction?cmd=remind&cooperationId=" + id +"&appName=" + appName + "&channelName=" + channelName + "&packetId=" + packetId;
}
function remind(id,appName,channelName,packetId){
	window.location="../servlet/CooperationRemindAction?cmd=remind&cooperationId=" + id +"&appName=" + appName + "&channelName=" + channelName + "&packetId=" + packetId;
}
function addAct(){
    window.location="cfgCooperationEdit.jsp?op=edit";
}
function editAct(id){
    window.location="cfgCooperationEdit.jsp?pageno=<%=pageno%>&op=edit&id="+id;
}
