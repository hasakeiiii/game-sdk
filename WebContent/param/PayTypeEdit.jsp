...
if(id!=0){
	...
	PayTypeCount payTypeCount=(PayTypeCount)payTypeCountDao.getById(id);
    name = payTypeCount.getName();
    no = payTypeCount.getNo();
    count = payTypeCount.getCount();
    payType = payTypeCount.getPayType();
    settleCount = payTypeCount.getSettleCount();
...
function btnSave(){
    ...
    if('<%=id%>'=="0"){
        document.mainForm.action="PayTypeAction.jsp?op=add";
        document.mainForm.submit();
    }
    else{//编辑功能
        document.mainForm.action="PayTypeAction.jsp?op=edit&pageno=<%=pageno%>&id=<%=id%>";
        document.mainForm.submit();
    }
}
</script>
...
<tr align="center">
    <td width="33%" id="title_no">编号</td>
    <td width="33%"><input name="no" alt="no" value="<%=no%>" maxlength="250"/></td>
<td>
<span id="warn_sp_name"></span></td>
</tr>
<tr align="center">
    <td width="33%" id="title_name">计费名称</td>
    <td width="33%"><input name="name" alt="name" value="<%=name%>" maxlength="250"/></td>
<td>
<span id="warn_sp_no"></span></td>
</tr>
<tr align="center">
    <td width="33%" id="title_count">显示金额百分比</td>
    <td width="33%"><input name="count" alt="count" value="<%=count%>" maxlength="250"/></td>
<td>
<span id="warn_sp_no"></span></td>
</tr>
<tr align="center">
    <td width="33%" id="title_settleCount">结算金额百分比</td>
    <td width="33%"><input name="settleCount" alt="settleCount" value="<%=settleCount%>" maxlength="250"/></td>
<td>
<span id="warn_sp_no"></span></td>
</tr>
<tr align="center">
    <td width="33%" id="title_payType">计费payType</td>
    <td width="33%"><input name="payType" alt="payType" value="<%=payType%>" maxlength="250"/></td>
<td>
<span id="warn_sp_no"></span></td>
</tr>
