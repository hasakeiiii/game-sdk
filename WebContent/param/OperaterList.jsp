...
for(int i=0;i<list.size();i++){
    Operater operater=(Operater)list.get(i);
    Userinfo userinfor = new Userinfo();
    userinfor.setId(0);
    userinfor.setUsername("");
    userinfor.setPass("");
    String role = "";
    if(operater.login_user != 0)
    {
	    for(int j = 0; j < userlist.size(); j ++)
	    {
	    	Userinfo tUser=(Userinfo)userlist.get(j);
	    	 ...
	    	 int id1 = tUser.id;
		 int id2 = operater.login_user;
		     if(id1 == id2)
	    	 {	    		
	    		userinfor = tUser;
                        role = (String)roleMap.get(userinfor.getRole());
	    		break;
	    	 }
	    }
    }
%>
<tr>
<td><%=operater.getId()%></td>
<td><%=operater.getName()%></td>
<td><%=operater.getNo()%></td>
<td><%=userinfor.getUsername()%></td>
<td><%=userinfor.getPass()%></td>
<td><%=role%></td>
<td><a href="#" onclick="editAct('<%=operater.getId()%>','<%=userinfor.getId()%>')"><img src="../_js/ico/btn_edit.gif" border="0" alt="修改"/></a></td>
</tr>
<%
}
%>
...
<script>
function addAct(){
    window.location="OperaterEdit.jsp";
}

function editAct(id,uerid){
    window.location="OperaterEdit.jsp?pageno=<%=pageno%>&id="+id+"&uerid="+uerid;
}
function infoAct(id){
...
function goSearch(){
	...
	document.mainForm.action="OperaterEdit.jsp";
