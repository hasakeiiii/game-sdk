...
String basePath = ConstValue.CPPACKETServerUrl+ "/";	
...
String pageSql="select * from gamepack";
if(!keyWord.equals("")){
...
List<Gamepack> list=(List<Gamepack>)apkDao.getList(pageno,pagesize,pageSql);
...
<td><%=apk.getSole()%></td>
<td><%=apk.getTime()%></td>
<td><a href="#" onclick="editAct('<%=apk.getId()%>')">审核状态</a></td>
</tr>
<%
}
%>
...
<script>
function addAct(){
    window.location="cfgCPApkEdit.jsp";
}
function editAct(id){
	window.location="cfgCPApkRunAction.jsp?op=edit&pageno=<%=pageno%>&id="+id;
}
function infoAct(id){
