...
String sessionSell=azul.JspUtil.getStr(session.getAttribute("sell"),"");
if("".equals(sessionSell)){
   %>
   <script>error("无法得到业务员编号");</script>
   <%
   return;
}
int cfg_sell_id=0;
...
CfgSell cfgSell=(CfgSell)cfgSellDao.loadBySql("select * from cfg_sell where cfg_sell_id="+sessionSell);
cfg_sell_id=Integer.valueOf(sessionSell);
...
function btnSave(){
    ...
    document.mainForm.action="cfgSellAction.jsp?op=editSelf&cfg_sell_id=<%=cfg_sell_id%>";
