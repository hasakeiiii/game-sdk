...
for(int i=0;i<list.size();i++){	
	    Businesser businesser=(Businesser)list.get(i);
	    Userinfo userinfor = new Userinfo();
	    userinfor.setId(0);
	    userinfor.setUsername("");
	    userinfor.setPass("");
	    if(businesser.login_user != 0)
	    {
...
%>
<tr>
<td><%=businesser.getId()%></td>
<td><%=businesser.getName()%></td>
<td><%=businesser.getNo()%></td>
<td><%=userinfor.getUsername()%></td>
<td><%=userinfor.getPass()%></td>
<td><a href="#" onclick="editAct('<%=businesser.getId()%>','<%=userinfor.getId()%>')"><img src="../_js/ico/btn_edit.gif" border="0" alt="修改"/></a></td>
</tr>
<%
}
%>
...
<script>
function addAct(){
    window.location="BusinesserEdit.jsp";
}

function editAct(id,uerid){
    window.location="BusinesserEdit.jsp?pageno=<%=pageno%>&id="+id+"&uerid="+uerid;
}
function infoAct(id){
...
function goSearch(){
	...
	document.mainForm.action="BusinesserList.jsp";
