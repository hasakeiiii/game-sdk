...
<%
DebuUtil.log("vaguelist");
VagueDao vagueDao=new VagueDao();
String keyWord=azul.JspUtil.getStr(request.getParameter("keyWord"),"");
String pageSql="select * from vague where 1=1";
if(!keyWord.equals("")){
	   pageSql+=" and no like '%"+keyWord+"%'";
}
int pagesize=30;//每页记录数
int pageno=azul.JspUtil.getInt(request.getParameter("pageno"),1);
int recordcount=vagueDao.getRecordCount(pageSql);//得到记录总数
DebuUtil.log("recordcount="+recordcount);
List<Vague> list=vagueDao.getList(pageno,pagesize,pageSql);
...
<td><%=vague.getNo()%></td>
<td><%=vague.getName()%></td>
<td><%=vague.getVaguePacket()%></td>
<td><a href="#" onclick="editAct('<%=vague.getId()%>')"><img src="../_js/ico/btn_edit.gif" border="0" alt="修改"/></a></td>
</tr>
<%
}
%>
...
<script>
function addAct(){
    window.location="VagueEdit.jsp";
}

function editAct(id){
    window.location="VagueEdit.jsp?pageno=<%=pageno%>&id="+id+"";
}
function infoAct(id){
...
function goSearch(){
	...
	document.mainForm.action="VagueList.jsp";
