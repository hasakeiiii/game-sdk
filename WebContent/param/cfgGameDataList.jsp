...
/*if(game_no != null)
{
	String sql = String.format("select channel_no,channel.name from cooperation,channel where app_no='%s' and channel.no=cooperation.channel_no GROUP BY channel_no",game_no);
	channelMap = cooperationDao.getSelectMap(sql);
}*/
{
String sql = String.format("select no,name from channel");
channelMap = cooperationDao.getSelectMap(sql);
}
...
if((!StringUtil.is_nullString(op)) &&(op.equals("sch")))
{
	...
	if(channel_id != -1)
	{
		...
		list = Gamedata.get_GamedataList("","","",cooperation.packet_id,cooperation.settle_type,"",beginDate,endDate);
	...
	else
	{
		...
		list = Gamedata.get_GamedataList(game_no,business_no,channel_no,"","","",beginDate,endDate);
	}
...
{
	//alert(obj.value);
	document.mainForm.action="cfgGameDataList.jsp?op=selgame";
...
{
	//alert(obj.value);
	document.mainForm.action="cfgGameDataList.jsp?op=selbusiness";
	document.mainForm.submit();
}

function channelchg(obj)
{
	document.mainForm.action="cfgGameDataList.jsp?op=selchannel";
	document.mainForm.submit();
}

function packetchg(obj)
{
	document.mainForm.action="cfgGameDataList.jsp?op=sel";
	document.mainForm.submit();
}
...
function goSearch(){
	...
	document.mainForm.action="cfgGameDataList.jsp?op=sch";
	document.mainForm.submit();
}
</script>
