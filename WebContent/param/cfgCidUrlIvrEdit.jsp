...
<%@ page import="model.*" %>
<%@ page import="azul.*" %>
<%@ include file="../check.jsp?check=cfgCidUrlIvrEdit.jsp"%>
<%
String pageTitle="编辑信息";
int pageno=JspUtil.getInt(request.getParameter("pageno"),1);
int cfg_cid_url_ivr_id=JspUtil.getInt(request.getParameter("cfg_cid_url_ivr_id"),0);
String cid="";//
String url="";//
if(cfg_cid_url_ivr_id!=0){
    pageTitle="编辑信息ID:"+cfg_cid_url_ivr_id;
    CfgCidUrlIvrDao cfgCidUrlIvrDao=new CfgCidUrlIvrDao();
    CfgCidUrlIvr cfgCidUrlIvr=(CfgCidUrlIvr)cfgCidUrlIvrDao.getById(cfg_cid_url_ivr_id);
    cid=cfgCidUrlIvr.getCid();
    url=cfgCidUrlIvr.getUrl();
}
...
function btnSave(){
    ...
    var paramKey="";
    if("<%=cfg_cid_url_ivr_id%>"=="0"){
        document.mainForm.action="cfgCidUrlIvrAction.jsp?op=add"+paramKey;
        document.mainForm.submit();
    }
    else{
        document.mainForm.action="cfgCidUrlIvrAction.jsp?op=edit&pageno=<%=pageno%>&cfg_cid_url_ivr_id=<%=cfg_cid_url_ivr_id%>"+paramKey;
        document.mainForm.submit();
    }
}
<%
