...
for(int i=0;i<list.size();i++){
    CfgSp cfgSp=(CfgSp)list.get(i);
%>
<tr>
<td><%=(pageno-1)*pagesize+i+1%></td>
<td><%=cfgSp.getSpName()%></td>
<td><%=cfgSp.getSpCode()%></td>
<td><a href="#" onclick="editAct('<%=cfgSp.getCfgSpId()%>')"><img src="../_js/ico/btn_edit.gif" border="0" alt="修改"/></a></td>
</tr>
<%
}
%>
...
<script>
function addAct(){
    window.location="cfgSpEdit.jsp";
}
function editAct(cfg_sp_id){
    window.location="cfgSpEdit.jsp?pageno=<%=pageno%>&cfg_sp_id="+cfg_sp_id;
}
...
function goSearch(){
	...
	document.mainForm.action="cfgSpList.jsp";
