...
<%
DebuUtil.log("gamePacManalist");
GamepackDao gpDao=new GamepackDao();
String keyWord=azul.JspUtil.getStr(request.getParameter("keyWord"),"");
String pageSql="select * from gamepack where 1=1";
if(!keyWord.equals("")){
	   pageSql+=" and game_name like '%"+keyWord+"%'";
}
pageSql+=" order by id desc";

int pagesize=30;//每页记录数
int pageno=azul.JspUtil.getInt(request.getParameter("pageno"),1);
int recordcount=gpDao.getRecordCount(pageSql);//得到记录总数
DebuUtil.log("recordcount="+recordcount);
List list = gpDao.getList(pageno,pagesize,pageSql);
...
<%
    ...
    Gamepack gamepack=(Gamepack)list.get(i);
%>
<tr>
<td><%=gamepack.getId()%></td>
<td><%=gamepack.getGameName()%></td>
<td><%=gamepack.getFileName()%></td>
<td><%=gamepack.getPacketNo()%></td>
<td><a href="#" onclick="editAct('<%=gamepack.getId()%>')"><img src="../_js/ico/btn_edit.gif" border="0" alt="修改"/></a></td>
</tr>
<%
}
%>
...
<script>
function addAct(){
    window.location="gamePacManaEdit.jsp";
}
function editAct(id){
	window.location="gamePacManaEdit.jsp?pageno=<%=pageno%>&id="+id;
}
function infoAct(id){
