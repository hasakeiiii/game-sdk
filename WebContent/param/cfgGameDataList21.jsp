...
<%
DebuUtil.log("cfgGameDataList21");
...
String bssSql = "select no,name from businesser";
...
if((!StringUtil.is_nullString(op)) &&(op.equals("sch")))//
{
	...
	if(channel_id != -1)
	{
		...
		list = GamePayData.get_GamedataList(game_no,"","",cooperation.packet_id,cooperation.settle_type,distype,beginDate,endDate);
	...
	else
	{
		...
		list = GamePayData.get_GamedataList(game_no,business_no,channel_no,"","",distype,beginDate,endDate);
...
{
	//alert(obj.value);
	document.mainForm.action="cfgGameDataList21.jsp?op=selgame&BSS_NO=<%=BSS_NO%>";
	document.mainForm.submit();
}
function businesschg(obj)
{
	//alert(obj.value);
	document.mainForm.action="cfgGameDataList21.jsp?op=selbusiness&BSS_NO=<%=BSS_NO%>";
	document.mainForm.submit();
}
function channelchg(obj)
{
	document.mainForm.action="cfgGameDataList21.jsp?op=selchannel&BSS_NO=<%=BSS_NO%>";
	document.mainForm.submit();
}

function packetchg(obj)
{
	document.mainForm.action="cfgGameDataList21.jsp?op=sel&BSS_NO=<%=BSS_NO%>";
	document.mainForm.submit();
}
...
function goSearch(){
	...
	document.mainForm.action="cfgGameDataList21.jsp?op=sch&BSS_NO=<%=BSS_NO%>";
	document.mainForm.submit();
}
</script>
