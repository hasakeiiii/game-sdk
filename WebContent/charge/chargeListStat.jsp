...
<%
String linkid=JspUtil.getStr(request.getParameter("linkid"),"");
...
if(begCal.get(Calendar.YEAR)*100+begCal.get(Calendar.MONTH)<endCal.get(Calendar.YEAR)*100+endCal.get(Calendar.MONTH)){
	tableName="charge_"+startDate.substring(0,7).replace("-","_");
}
...
if(!"".equals(linkid)){
	conditionStrSB.append(" and linkid='");
	conditionStrSB.append(linkid);
	conditionStrSB.append("'");
}
if(!"".equals(area)){
...
List list=chargeDao.getList(pageSql);
%>
<body>
...
<td>LinkId&nbsp;
  <input name="linkid" type="text" id="linkid" value="<%=linkid%>" style="width:100px"/></td>
<td>手机号码&nbsp;
...
function goSearch(){
    document.mainForm.action="chargeListStat.jsp";
