...
StringBuffer conditionStrSB=new StringBuffer("select DATE(date_time),sum(case when operator=1 then 1 else 0 end),sum(case when operator=1 then fee else 0 end),sum(case when operator=2 then 1 else 0 end),sum(case when operator=2 then fee else 0 end) from "+tableName+" where ok=1");
...
String linkParam="cid="+cid+"&sid="+sid+"&msg="+msg+"&fee_type="+fee_type+"&province="+province_code;
...
&nbsp;&nbsp;通道命令&nbsp;&nbsp;
<input name="msg" type="text" id="msg" value="<%=msg%>" style="width:70px"/>
&nbsp;&nbsp;合作厂商&nbsp;&nbsp;
...
<a href="#" onclick="_jsSearch('chargeListMonth.jsp')"><img src="../_js/ico/btn_search.gif" border="0" alt="搜索" align="absmiddle"/></a>
&nbsp;</td>
</tr>
</table>
...
for(int i=0;i<list.size();i++){ 
	...
	int count2=Integer.valueOf(tempList.get(3).toString());
	int sum2=Integer.valueOf(tempList.get(4).toString());
	allCount1+=count1;
	allCount2+=count2;
	allSum1+=sum1;
	allSum2+=sum2;
